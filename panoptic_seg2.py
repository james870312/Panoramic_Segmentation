...
parser.add_argument('-nps', default=5, type=int,
...
def main():
    ...
    for batch_idx, (data, target) in tqdm.tqdm(enumerate(val_loader),
        ...
        for img, lt, lp in zip(imgs, lbl_true, lbl_pred):
            ...
            for i in range(masks.shape[2]):
                ...
                #cv2.imshow("FCN_Compare", viz)
                kbin = 0#cv2.waitKey(0)

                if kbin==27:
                    break
                if kbin == 99:
                    skimage.io.imsave('img/output1_'+str(num)+'.png', res)
                    skimage.io.imsave('img/output2_'+str(num)+'.png', result)
                    skimage.io.imsave('img/output3_'+str(num)+'.png', img_pseg)
    ...
    df.to_csv("voc_coco2.csv", index=False)
