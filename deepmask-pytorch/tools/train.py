...
#                    help='data set')
parser.add_argument('--dataset', default='city', choices=dataset_names(),
                    help='data set')
...
class IouMeter(object):
    ...
    def add(self, output, target):
        ...
        mask_sum = pred.eq(1).to(torch.int)+target.eq(1).to(torch.int)
        #print("#####",target[target==1])
        #print("######",target.eq(1).to(torch.int))
        #print("$$$",pred.shape,target.shape,mask_sum.shape)
        #print("$$$",mask_sum[mask_sum==2])
        intxn = torch.sum(mask_sum ==2, dim=(1, 2)).float()
        union = torch.sum(mask_sum > 0, dim=(1, 2)).float()
        for i in range(batch):
            if union[i].item() > 0:
                #print("@@@",intxn[i].item(), union[i].item())
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (img, target, head_status) in enumerate(train_loader):
        ...
        #print(test)
        #skimage.io.imsave('test.jpg', test)
        ...
        # compute output
        output = model(img)
        #output, test2 = model(img)

        #test=output[0].cpu().detach()
        #test = test.permute(0,2,3,1)
        #test = test.numpy()
        #print("mask_size",np.shape(output[0].cpu()))
        #print("score_size",np.shape(output[1].cpu()))
        #plt.imshow(test[0])
        #plt.show()
