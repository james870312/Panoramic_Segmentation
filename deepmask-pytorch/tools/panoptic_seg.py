...
parser.add_argument('--img', default='data/test',
                    help='path/to/test/image')
...
def main():
    ...
    img_path = os.listdir(args.img)
    img_path.sort()
    #img_path=["2007_003201.jpg", "2007_003201_4.jpg"]
    #print(args.img)
    #print(img_path)
    #print(len(img_path))
    #for j in range(int((len(img_path)/3))):
    for j in range(int(len(img_path))):
        print("#",j)
    
        print('| start'); tic = time.time()
        im = np.array(Image.open(args.img+"/"+img_path[j]).convert('RGB'), dtype=np.float32)
        #cv2.imshow('test', im)
        #cv2.waitKey(0)
        h, w = im.shape[:2]
