...
_config = _Config(datadir='/home/james/data/datasets/cityscapes',
...
class cityDataset(data.Dataset):
    ...
    def __init__(self, config=None, split='val'):
        ...
        #print("config.datadir = ", config.datadir)
        self.datadir = config.datadir if hasattr(config, 'datadir') else join('/home', 'james','data','datasets', 'cityscapes')
        #print("config.datadir = ", self.datadir)
        self.split = split
        self.annFile = join(self.datadir, 'annotations/instancesonly_filtered_gtFine_%s.json' % split)
